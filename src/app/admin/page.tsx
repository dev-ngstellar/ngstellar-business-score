...
export default function AdminDashboard() {
  ...
  const legalBadge = (status?: string) => {
    if (status === 'Strong') return 'bg-emerald-500/10 text-emerald-400 border-emerald-500/30';
    if (status === 'Good') return 'bg-sky-500/10 text-sky-400 border-sky-500/30';
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs sm:text-sm text-slate-400 font-medium">
                  View and manage submitted business health assessments and lead responses.
                </p>
              </div>
            </div>
          </div>
                        ...
                        <td className="px-5 py-4">
                          <div className="space-y-1 text-xs">
                            ...
                            {item.industrySubCategory && (
                              <span className="inline-block px-2 py-0.5 rounded text-[10px] font-bold bg-slate-800 text-slate-300 border border-white/10">
                                {item.industrySubCategory}
                              </span>
                            )}
          ...
          <div className="w-full max-w-4xl max-h-[92vh] flex flex-col bg-slate-900 rounded-3xl border border-white/15 shadow-2xl text-slate-100 overflow-hidden">
            ...
            <div className="px-6 py-5 border-b border-white/10 bg-slate-950/60 flex items-center justify-between shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-extrabold text-white text-lg sm:text-xl">Business Health Report Details</h3>
                    ...
                    <div className="shrink-0 flex items-center gap-4 bg-slate-950/80 p-4 rounded-2xl border border-white/10 self-stretch sm:self-auto justify-center">
                      ...
                      {detailData.legalComplianceScore != null && (
                        <div className="border-l border-white/10 pl-4 text-center">
                          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Legal Compliance</p>
                          <p className="text-2xl font-black text-emerald-400">{detailData.legalComplianceScore}%</p>
                          <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${legalBadge(detailData.legalComplianceStatus)}`}>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-4 rounded-2xl bg-slate-950/50 border border-white/10">
                    <div>
                      ...
                      {detailData.industrySubCategory && (
                        <span className="block text-[10px] text-sky-400">{detailData.industrySubCategory}</span>
                      )}
                    </div>
                    <div>
                    ...
                    {detailData.gstRegistered && (
                      <div>
                        <span className="text-slate-400 text-xs block">GST Registered</span>
                        <strong className="text-white font-semibold text-xs">{detailData.gstRegistered}</strong>
                      </div>
                    )}
                    {detailData.trademarkRegistered && (
                      <div>
                        <span className="text-slate-400 text-xs block">Trademark Status</span>
                        <strong className="text-white font-semibold text-xs">{detailData.trademarkRegistered}</strong>
                      </div>
                    )}
                  </div>
